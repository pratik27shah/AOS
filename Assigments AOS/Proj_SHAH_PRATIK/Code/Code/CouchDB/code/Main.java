...
public static final Hashtable<Integer,Session> GetCouch=new Hashtable<Integer,Session>();
...
//BasicDBObject odb;
public static final Hashtable<Integer,Database> GetCouchDb = new Hashtable<Integer,Database>();
...
Get_Server_IP_Hash objGet_Server_IP_Hash=new Get_Server_IP_Hash();
  ...
  ReadConfig objReadConfig=new ReadConfig();
  //Document Couchdoc = new Document();
		int  Counter=0;
...
for(int i=0;i<Total_Servers;i++)
{
   ...
   Session s1=new Session(ArrayList_ServerList.get(i),5984);
    //System.out.println();
    //System.out.println(ArrayList_ServerList.get(i));
    GetCouch.put(i,s1);
    GetCouch.get(i).createDatabase("databases");
GetCouchDb.put(i,GetCouch.get(i).getDatabase("databases"));
    //GetCouch.put(i, new Jedis(ArrayList_ServerList.get(Hash_Value)));
System.out.println("praitk");
}
Session couch;
/*Insert*/
...
//jedis=GetRedis.get(Hash_Value);
 Document Couchdoc = new Document();
Couchdoc.put(Hash_keys.get((Counter)), Valuetoadd);
Couchdoc.setId(Hash_keys.get((Counter)));
GetCouchDb.get(Hash_Value).saveDocument(Couchdoc);
//Couchdoc=null;
 Counter++;
...
//Couchdoc.put(Hash_keys.get((Counter)), Valuetoadd);
Couchdoc=GetCouchDb.get(Hash_Value).getDocument(Hash_keys.get((Counter)));
//Couchdoc=null;
 Counter++;
...
Couchdoc=GetCouchDb.get(Hash_Value).getDocument(Hash_keys.get((Counter)).toString());
GetCouchDb.get(Hash_Value).deleteDocument(Couchdoc);
 Counter++;
...
if(objConnection!=null){
	...
	*/
     public void Sockets_Set_Connection()
    {
	  ...
	  @Override
/*Purpose:Sends file to client(redis) requesting it */
