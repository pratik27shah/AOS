...
public static final Hashtable<Integer,Jedis> GetRedis=new Hashtable<Integer,Jedis>();
...
{
GetRedis.put(i, new Jedis(ArrayList_ServerList.get(Hash_Value)));
}
Jedis jedis;
/*Insert*/
...
jedis.set(Hash_keys.get((Counter)), Valuetoadd);
 Counter++;
...
while(true)
{
   Hash_Value=Hash_Selected_Value.get(Hash_keys.get(Counter));//HashFunction(Key,Total_Servers);
jedis=GetRedis.get(Hash_Value);
jedis.get(Hash_keys.get((Counter)));
 Counter++;
 if(Counter>=UpperValue-1) break;   
}
...
while(true)
{
     Hash_Value=Hash_Selected_Value.get(Hash_keys.get(Counter));//HashFunction(Key,Total_Servers);
jedis=GetRedis.get(Hash_Value);
jedis.del(Hash_keys.get((Counter)));
